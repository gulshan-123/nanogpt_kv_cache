    ...
    with ctx:
        for k in range(num_samples):
            ...
            for max_new_token in max_token:
                ...
                tot_inference_time.append(np.sum(per_loop_time))
            plt.plot(max_token, avg_per_loop)
            plt.title("Time for running inference")
            plt.xlabel("Token size")
            plt.ylabel("Time per loop(in seconds)")
            plt.show()
            plt.plot(max_token, tot_inference_time)
            plt.title("Time for running inference")
            plt.xlabel("Token size")
            plt.ylabel("Total Time for inference(in seconds)")
            plt.show()
            print(decode(y))
            print('---------------')
